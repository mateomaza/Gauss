...
function getValues() {
  ...
  // We make sure both values aren't empty (empty inputs pass as empty arrays)
  // Technically, the second statements isn't necessary, as eValue = 0 won't work, so just for comprehension
  if (sValue !== '' && typeof eValue !== '') {
  ...
  } else {alert("Start Value must be an integer between 0 & 99, End Value must be an integer between 1 & 100")}
}
